...
{
        return d_is_positive(dentry) && !d_unhashed(dentry);
}
				...
				void (*release)(struct kref *))
{
	...
	if (!parent || d_is_negative(parent))
		goto out;
	/* serialize with d_delete() */
	mutex_lock(&d_backing_inode(parent)->i_mutex);
	/* Make sure the object is still alive */
	if (debugfs_positive(file->f_path.dentry)
	&& kref_get_unless_zero(kref))
		ret = 0;
	mutex_unlock(&d_backing_inode(parent)->i_mutex);
	if (!ret) {
