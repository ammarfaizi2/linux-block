				 ...
				 struct hisi_pmu *pa_pmu)
{
	...
	pa_pmu->pmu.num_counters = PA_NR_COUNTERS;
	pa_pmu->ops = &hisi_uncore_pa_ops;
	pa_pmu->check_event = 0xB0;
	pa_pmu->counter_bits = 64;
	pa_pmu->pmu.dev = &pdev->dev;
...
static int hisi_pa_pmu_probe(struct platform_device *pdev)
{
	...
	return uncore_pmu_register(&pa_pmu->pmu, name);
}
...
static int hisi_pa_pmu_remove(struct platform_device *pdev)
{
	...
	uncore_pmu_unregister(&pa_pmu->pmu);
	return 0;
}
