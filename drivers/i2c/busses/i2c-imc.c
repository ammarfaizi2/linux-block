			  ...
			  int size, union i2c_smbus_data *data)
{
	...
	case I2C_SMBUS_WORD_DATA:
		...
		if (read_write == I2C_SMBUS_READ)
			...
			cmdbits |= SMBCMD_TYPE_WRITE | cpu_to_be16(data->word);
		break;
	default:
		return -EOPNOTSUPP;
	}
...
static int imc_init_channel(struct imc_priv *priv, int i, int socket)
{
	...
	ch->adapter.class = I2C_CLASS_SPD;  /* Don't probe for sensors. */
	...
	/*
	 * If an old-style driver like eeprom is already loaded, then
	 * imc_smbus_xfer will be called immediately.  We need to be
	 * ready.
	 */
	mutex_init(&ch->mutex);

	snprintf(ch->adapter.name, sizeof(ch->adapter.name),
		 "iMC SMBUS, socket %d (bus %x) channel %d",
		 socket, priv->pci_dev->bus->number, i);
