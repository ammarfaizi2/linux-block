...
static bool ath12k_hw_qcn92xx_rx_desc_get_first_msdu(struct hal_rx_desc *desc)
{
	return !!FIELD_GET(RX_MSDU_END_INFO5_FIRST_MSDU,
			   __le16_to_cpu(desc->u.qcn92xx.msdu_end.info5));
}

static bool ath12k_hw_qcn92xx_rx_desc_get_last_msdu(struct hal_rx_desc *desc)
{
	return !!FIELD_GET(RX_MSDU_END_INFO5_LAST_MSDU,
			   __le16_to_cpu(desc->u.qcn92xx.msdu_end.info5));
}

static u8 ath12k_hw_qcn92xx_rx_desc_get_l3_pad_bytes(struct hal_rx_desc *desc)
{
	return FIELD_GET(RX_MSDU_END_INFO5_L3_HDR_PADDING,
			 __le16_to_cpu(desc->u.qcn92xx.msdu_end.info5));
}

static bool ath12k_hw_qcn92xx_rx_desc_encrypt_valid(struct hal_rx_desc *desc)
{
	return !!FIELD_GET(RX_MPDU_START_INFO4_ENCRYPT_INFO_VALID,
			   __le32_to_cpu(desc->u.qcn92xx.mpdu_start.info4));
}

static u32 ath12k_hw_qcn92xx_rx_desc_get_encrypt_type(struct hal_rx_desc *desc)
{
	return FIELD_GET(RX_MPDU_START_INFO2_ENC_TYPE,
			 __le32_to_cpu(desc->u.qcn92xx.mpdu_start.info2));
}

static u8 ath12k_hw_qcn92xx_rx_desc_get_decap_type(struct hal_rx_desc *desc)
{
	return FIELD_GET(RX_MSDU_END_INFO11_DECAP_FORMAT,
			 __le32_to_cpu(desc->u.qcn92xx.msdu_end.info11));
}

static u8 ath12k_hw_qcn92xx_rx_desc_get_mesh_ctl(struct hal_rx_desc *desc)
{
	return FIELD_GET(RX_MSDU_END_INFO11_MESH_CTRL_PRESENT,
			 __le32_to_cpu(desc->u.qcn92xx.msdu_end.info11));
}

static bool ath12k_hw_qcn92xx_rx_desc_get_mpdu_seq_ctl_vld(struct hal_rx_desc *desc)
{
	return !!FIELD_GET(RX_MPDU_START_INFO4_MPDU_SEQ_CTRL_VALID,
			   __le32_to_cpu(desc->u.qcn92xx.mpdu_start.info4));
}

static bool ath12k_hw_qcn92xx_rx_desc_get_mpdu_fc_valid(struct hal_rx_desc *desc)
{
	return !!FIELD_GET(RX_MPDU_START_INFO4_MPDU_FCTRL_VALID,
			   __le32_to_cpu(desc->u.qcn92xx.mpdu_start.info4));
}

static u16 ath12k_hw_qcn92xx_rx_desc_get_mpdu_start_seq_no(struct hal_rx_desc *desc)
{
	return FIELD_GET(RX_MPDU_START_INFO4_MPDU_SEQ_NUM,
			 __le32_to_cpu(desc->u.qcn92xx.mpdu_start.info4));
}

static u16 ath12k_hw_qcn92xx_rx_desc_get_msdu_len(struct hal_rx_desc *desc)
{
	return FIELD_GET(RX_MSDU_END_INFO10_MSDU_LENGTH,
			 __le32_to_cpu(desc->u.qcn92xx.msdu_end.info10));
}

static u8 ath12k_hw_qcn92xx_rx_desc_get_msdu_sgi(struct hal_rx_desc *desc)
{
	return FIELD_GET(RX_MSDU_END_INFO12_SGI,
			 __le32_to_cpu(desc->u.qcn92xx.msdu_end.info12));
}

static u8 ath12k_hw_qcn92xx_rx_desc_get_msdu_rate_mcs(struct hal_rx_desc *desc)
{
	return FIELD_GET(RX_MSDU_END_INFO12_RATE_MCS,
			 __le32_to_cpu(desc->u.qcn92xx.msdu_end.info12));
}

static u8 ath12k_hw_qcn92xx_rx_desc_get_msdu_rx_bw(struct hal_rx_desc *desc)
{
	return FIELD_GET(RX_MSDU_END_INFO12_RECV_BW,
			 __le32_to_cpu(desc->u.qcn92xx.msdu_end.info12));
}

static u32 ath12k_hw_qcn92xx_rx_desc_get_msdu_freq(struct hal_rx_desc *desc)
{
	return __le32_to_cpu(desc->u.qcn92xx.msdu_end.phy_meta_data);
}

static u8 ath12k_hw_qcn92xx_rx_desc_get_msdu_pkt_type(struct hal_rx_desc *desc)
{
	return FIELD_GET(RX_MSDU_END_INFO12_PKT_TYPE,
			 __le32_to_cpu(desc->u.qcn92xx.msdu_end.info12));
}

static u8 ath12k_hw_qcn92xx_rx_desc_get_msdu_nss(struct hal_rx_desc *desc)
{
	return FIELD_GET(RX_MSDU_END_INFO12_MIMO_SS_BITMAP,
			 __le32_to_cpu(desc->u.qcn92xx.msdu_end.info12));
}

static u8 ath12k_hw_qcn92xx_rx_desc_get_mpdu_tid(struct hal_rx_desc *desc)
{
	return FIELD_GET(RX_MSDU_END_INFO5_TID,
			 __le16_to_cpu(desc->u.qcn92xx.msdu_end.info5));
}

static u16 ath12k_hw_qcn92xx_rx_desc_get_mpdu_peer_id(struct hal_rx_desc *desc)
{
	return __le16_to_cpu(desc->u.qcn92xx.mpdu_start.sw_peer_id);
}

static void ath12k_hw_qcn92xx_rx_desc_copy_end_tlv(struct hal_rx_desc *fdesc,
						   struct hal_rx_desc *ldesc)
{
	memcpy((u8 *)&fdesc->u.qcn92xx.msdu_end, (u8 *)&ldesc->u.qcn92xx.msdu_end,
...
static void ath12k_hw_qcn92xx_rx_desc_set_msdu_len(struct hal_rx_desc *desc, u16 len)
{
	u32 info = __le32_to_cpu(desc->u.qcn92xx.msdu_end.info10);

	info &= ~RX_MSDU_END_INFO10_MSDU_LENGTH;
	info |= FIELD_PREP(RX_MSDU_END_INFO10_MSDU_LENGTH, len);

	desc->u.qcn92xx.msdu_end.info10 = __cpu_to_le32(info);
}

static u8 *ath12k_hw_qcn92xx_rx_desc_get_msdu_payload(struct hal_rx_desc *desc)
{
	return &desc->u.qcn92xx.msdu_payload[0];
}
...
{
	return __le32_to_cpu(desc->u.qcn92xx.mpdu_start.info4) &
	       RX_MPDU_START_INFO4_MAC_ADDR2_VALID;
}

static u8 *ath12k_hw_qcn92xx_rx_desc_mpdu_start_addr2(struct hal_rx_desc *desc)
{
	return desc->u.qcn92xx.mpdu_start.addr2;
}

static bool ath12k_hw_qcn92xx_rx_desc_is_mcbc(struct hal_rx_desc *desc)
{
	return __le32_to_cpu(desc->u.qcn92xx.mpdu_start.info6) &
	       RX_MPDU_START_INFO6_MCAST_BCAST;
}

static void ath12k_hw_qcn92xx_rx_desc_get_dot11_hdr(struct hal_rx_desc *desc,
						    struct ieee80211_hdr *hdr)
{
	hdr->frame_control = desc->u.qcn92xx.mpdu_start.frame_ctrl;
	hdr->duration_id = desc->u.qcn92xx.mpdu_start.duration;
	ether_addr_copy(hdr->addr1, desc->u.qcn92xx.mpdu_start.addr1);
	ether_addr_copy(hdr->addr2, desc->u.qcn92xx.mpdu_start.addr2);
	ether_addr_copy(hdr->addr3, desc->u.qcn92xx.mpdu_start.addr3);
	if (__le32_to_cpu(desc->u.qcn92xx.mpdu_start.info4) &
			RX_MPDU_START_INFO4_MAC_ADDR4_VALID) {
		ether_addr_copy(hdr->addr4, desc->u.qcn92xx.mpdu_start.addr4);
	}
	hdr->seq_ctrl = desc->u.qcn92xx.mpdu_start.seq_ctrl;
}

static void ath12k_hw_qcn92xx_rx_desc_get_crypto_hdr(struct hal_rx_desc *desc,
						     u8 *crypto_hdr,
						     enum hal_encrypt_type enctype)
{
	...
	case HAL_ENCRYPT_TYPE_TKIP_MIC:
		crypto_hdr[0] =
			HAL_RX_MPDU_INFO_PN_GET_BYTE2(desc->u.qcn92xx.mpdu_start.pn[0]);
		crypto_hdr[1] = 0;
		crypto_hdr[2] =
			HAL_RX_MPDU_INFO_PN_GET_BYTE1(desc->u.qcn92xx.mpdu_start.pn[0]);
		break;
	...
	key_id        = FIELD_GET(RX_MPDU_START_INFO5_KEY_ID,
				  __le32_to_cpu(desc->u.qcn92xx.mpdu_start.info5));
	crypto_hdr[3] = 0x20 | (key_id << 6);
	crypto_hdr[4] = HAL_RX_MPDU_INFO_PN_GET_BYTE3(desc->u.qcn92xx.mpdu_start.pn[0]);
	crypto_hdr[5] = HAL_RX_MPDU_INFO_PN_GET_BYTE4(desc->u.qcn92xx.mpdu_start.pn[0]);
	crypto_hdr[6] = HAL_RX_MPDU_INFO_PN_GET_BYTE1(desc->u.qcn92xx.mpdu_start.pn[1]);
	crypto_hdr[7] = HAL_RX_MPDU_INFO_PN_GET_BYTE2(desc->u.qcn92xx.mpdu_start.pn[1]);
}

static u16 ath12k_hw_qcn92xx_rx_desc_get_mpdu_frame_ctl(struct hal_rx_desc *desc)
{
	return __le16_to_cpu(desc->u.qcn92xx.mpdu_start.frame_ctrl);
}

static int ath12k_hal_srng_create_config_qcn92xx(struct ath12k_base *ab)
{
...
static const struct hal_ops hal_qcn92xx_ops = {
	...
	.rx_desc_get_l3_pad_bytes = ath12k_hw_qcn92xx_rx_desc_get_l3_pad_bytes,
	.rx_desc_encrypt_valid = ath12k_hw_qcn92xx_rx_desc_encrypt_valid,
	.rx_desc_get_encrypt_type = ath12k_hw_qcn92xx_rx_desc_get_encrypt_type,
	.rx_desc_get_decap_type = ath12k_hw_qcn92xx_rx_desc_get_decap_type,
	.rx_desc_get_mesh_ctl = ath12k_hw_qcn92xx_rx_desc_get_mesh_ctl,
	.rx_desc_get_mpdu_seq_ctl_vld = ath12k_hw_qcn92xx_rx_desc_get_mpdu_seq_ctl_vld,
	.rx_desc_get_mpdu_fc_valid = ath12k_hw_qcn92xx_rx_desc_get_mpdu_fc_valid,
	.rx_desc_get_mpdu_start_seq_no = ath12k_hw_qcn92xx_rx_desc_get_mpdu_start_seq_no,
	.rx_desc_get_msdu_len = ath12k_hw_qcn92xx_rx_desc_get_msdu_len,
	.rx_desc_get_msdu_sgi = ath12k_hw_qcn92xx_rx_desc_get_msdu_sgi,
	.rx_desc_get_msdu_rate_mcs = ath12k_hw_qcn92xx_rx_desc_get_msdu_rate_mcs,
	.rx_desc_get_msdu_rx_bw = ath12k_hw_qcn92xx_rx_desc_get_msdu_rx_bw,
	.rx_desc_get_msdu_freq = ath12k_hw_qcn92xx_rx_desc_get_msdu_freq,
	.rx_desc_get_msdu_pkt_type = ath12k_hw_qcn92xx_rx_desc_get_msdu_pkt_type,
	.rx_desc_get_msdu_nss = ath12k_hw_qcn92xx_rx_desc_get_msdu_nss,
	...
	.rx_desc_get_mpdu_start_offset = ath12k_hw_qcn92xx_rx_desc_get_mpdu_start_offset,
	.rx_desc_get_msdu_end_offset = ath12k_hw_qcn92xx_rx_desc_get_msdu_end_offset,
	.rx_desc_mac_addr2_valid = ath12k_hw_qcn92xx_rx_desc_mac_addr2_valid,
	.rx_desc_mpdu_start_addr2 = ath12k_hw_qcn92xx_rx_desc_mpdu_start_addr2,
	.rx_desc_is_mcbc = ath12k_hw_qcn92xx_rx_desc_is_mcbc,
	.rx_desc_get_dot11_hdr = ath12k_hw_qcn92xx_rx_desc_get_dot11_hdr,
					...
					struct hal_srng *srng)
{
	...
	*srng->u.src_ring.tp_addr = 0;
	...
	if (srng->ring_id == HAL_SRNG_RING_ID_WBM_IDLE_LINK)
		val |= HAL_TCL1_RING_MISC_MSI_LOOPID_DISABLE;
