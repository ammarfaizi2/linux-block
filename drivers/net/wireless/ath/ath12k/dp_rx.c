...
{
	if (!ab->hal.ops->rx_desc_encrypt_valid(desc))
		return HAL_ENCRYPT_TYPE_OPEN;

	return ab->hal.ops->rx_desc_get_encrypt_type(desc);
}

u8 ath12k_dp_rx_h_decap_type(struct ath12k_base *ab,
			     struct hal_rx_desc *desc)
{
	return ab->hal.ops->rx_desc_get_decap_type(desc);
}

static u8 ath12k_dp_rx_h_mesh_ctl_present(struct ath12k_base *ab,
					  struct hal_rx_desc *desc)
{
	return ab->hal.ops->rx_desc_get_mesh_ctl(desc);
}

static bool ath12k_dp_rx_h_seq_ctrl_valid(struct ath12k_base *ab,
					  struct hal_rx_desc *desc)
{
	return ab->hal.ops->rx_desc_get_mpdu_seq_ctl_vld(desc);
}

static bool ath12k_dp_rx_h_fc_valid(struct ath12k_base *ab,
				    struct hal_rx_desc *desc)
{
	return ab->hal.ops->rx_desc_get_mpdu_fc_valid(desc);
}
...
bool ath12k_dp_rx_h_first_msdu(struct ath12k_base *ab,
			       struct hal_rx_desc *desc)
{
	return ab->hal.ops->rx_desc_get_first_msdu(desc);
}

static bool ath12k_dp_rx_h_last_msdu(struct ath12k_base *ab,
				     struct hal_rx_desc *desc)
{
	return ab->hal.ops->rx_desc_get_last_msdu(desc);
}
...
static bool ath12k_dp_rxdesc_mac_addr2_valid(struct ath12k_base *ab,
					     struct hal_rx_desc *desc)
{
	return ab->hal.ops->rx_desc_mac_addr2_valid(desc);
}

static u8 *ath12k_dp_rxdesc_get_mpdu_start_addr2(struct ath12k_base *ab,
						 struct hal_rx_desc *desc)
{
	return ab->hal.ops->rx_desc_mpdu_start_addr2(desc);
}
	...
	if (ar->ab->hw_params.rx_mac_buf_ring) {
		for (i = 0; i < ab->hw_params.num_rxmda_per_pdev; i++) {
			ret = ath12k_dp_srng_setup(ar->ab,
						   ...
						   mac_id + i, 1024);
			if (ret) {
				ath12k_warn(ar->ab, "failed to setup rx_mac_buf_ring %d\n",
					    i);
				return ret;
			}
		}
			       ...
			       struct ath12k_peer *peer, u8 tid)
{
	...
	ret = ath12k_dp_tx_send_reo_cmd(ar->ab, rx_tid,
...
static void ath12k_dp_rx_frags_cleanup(struct dp_rx_tid *rx_tid, bool rel_link_desc)
{
...
void ath12k_peer_rx_tid_cleanup(struct ath12k *ar, struct ath12k_peer *peer)
{
	...
	for (i = 0; i <= IEEE80211_NUM_TIDS; i++) {
		...
		ath12k_peer_rx_tid_delete(ar, peer, i);
			     ...
			     struct ieee80211_ampdu_params *params)
{
	...
	ret = ath12k_peer_rx_tid_setup(ar, params->sta->addr, vdev_id,
...
static int ath12k_dp_rx_h_defrag_reo_reinject(struct ath12k *ar, struct dp_rx_tid *rx_tid,
					      struct sk_buff *defrag_skb)
{
	struct ath12k_base *ab = ar->ab;
	struct ath12k_dp *dp = &ab->dp;
...
static bool
ath12k_dp_rx_h_defrag_validate_incr_pn(struct ath12k *ar, struct dp_rx_tid *rx_tid)
{
	struct ath12k_base *ab = ar->ab;
...
static int ath12k_dp_rxdma_ring_sel_config(struct ath12k_base *ab)
{
	struct ath12k_dp *dp = &ab->dp;
	...
	tlv_filter.rx_mpdu_start_offset =
			ab->hal.ops->rx_desc_get_mpdu_start_offset();
	tlv_filter.rx_msdu_end_offset = ab->hal.ops->rx_desc_get_msdu_end_offset();
