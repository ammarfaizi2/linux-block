...
static int ath12k_acpi_set_power_limit(struct ath12k_base *ab)
{
	...
	if (ab->acpi.tas_sar_power_table[0] != ATH12K_ACPI_TAS_DATA_VERSION ||
	    ab->acpi.tas_sar_power_table[1] != ATH12K_ACPI_TAS_DATA_ENABLE) {
		ath12k_warn(ab, "latest ACPI TAS data is invalid\n");
		return -EINVAL;
	}

	ret = ath12k_wmi_pdev_set_tas_data_table_param(ab, tas_sar_power_table);
	if (ret) {
		ath12k_warn(ab, "failed to pass ACPI TAS data table: %d\n", ret);
		return ret;
	}
...
static int ath12k_acpi_set_bios_sar_power(struct ath12k_base *ab)
{
	...
	ret = ath12k_wmi_pdev_set_bios_sar_table_param(ab, ab->acpi.bios_sar_data);
	if (ret) {
...
static int ath12k_acpi_set_bios_sar_params(struct ath12k_base *ab)
{
	...
	ret = ath12k_wmi_pdev_set_bios_geo_table_param(ab, ab->acpi.geo_offset_data);
	if (ret) {
...
static int ath12k_acpi_set_tas_params(struct ath12k_base *ab)
{
	...
	ret = ath12k_wmi_pdev_set_tas_cfg_table_param(ab, ab->acpi.tas_cfg);
	if (ret) {
	...
	ret = ath12k_wmi_pdev_set_tas_data_table_param(ab, ab->acpi.tas_sar_power_table);
	if (ret) {
...
int ath12k_acpi_register(struct ath12k_base *ab)
{
	...
	if (ret) {
		ath12k_warn(ab, "failed to get ACPI DSM data: %d\n", ret);
		return ret;
	}
