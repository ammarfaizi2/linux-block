				 ...
				 int offset, bool read)
{
	...
	rc = read ?
	     hv_read_config_block(dev->pdev, buf,
				  HV_CONFIG_BLOCK_SIZE_MAX, block_id,
				  &bytes_returned) :
	     hv_write_config_block(dev->pdev, buf,
				   HV_CONFIG_BLOCK_SIZE_MAX, block_id);
...
int mlx5_hv_register_invalidate(struct mlx5_core_dev *dev, void *context,
				void (*block_invalidate)(void *context,
							 u64 block_mask))
{
	return hv_register_block_invalidate(dev->pdev, context,
					    block_invalidate);
}

void mlx5_hv_unregister_invalidate(struct mlx5_core_dev *dev)
{
	hv_register_block_invalidate(dev->pdev, NULL, NULL);
}
