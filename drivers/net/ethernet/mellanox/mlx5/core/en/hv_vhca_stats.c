...
struct mlx5e_hv_vhca_per_ring_stats {
	...
	u64     tx_packets;
	u64     tx_bytes;
#define MLX5E_HV_VHCA_NUM_STATS 4
};
...
static void mlx5e_hv_vhca_fill_stats(struct mlx5e_priv *priv, u64 *data,
				     int buf_len)
{
	int num_stats = buf_len / sizeof(u64);
	int ch, i = 0;

	for (ch = 0; ch < mlx5e_get_netdev_max_channels(priv->netdev); ch++) {
		u64 *buf = data + i;

		if (WARN_ON(i + MLX5E_HV_VHCA_NUM_STATS > num_stats))
			return;

		mlx5e_hv_vhca_fill_ring_stats(priv, ch,
					      (struct mlx5e_hv_vhca_per_ring_stats *)buf);
		i += MLX5E_HV_VHCA_NUM_STATS;
	}
}

static int mlx5e_hv_vhca_stats_buf_size(struct mlx5e_priv *priv)
{
	return (MLX5E_HV_VHCA_NUM_STATS) *
		mlx5e_get_netdev_max_channels(priv->netdev) *
		sizeof(u64);
}
...
static void mlx5e_hv_vhca_stats_work(struct work_struct *work)
{
	...
	struct mlx5e_hv_vhca_stats_agent *sagent =
		container_of(dwork, struct mlx5e_hv_vhca_stats_agent, work);
	struct mlx5e_priv *priv =
		container_of(sagent, struct mlx5e_priv, stats_agent);
	struct mlx5_hv_vhca_agent *agent = sagent->agent;
					...
					struct mlx5_hv_vhca_control_block *block)
{
	...
	struct mlx5e_hv_vhca_stats_agent *sagent = &priv->stats_agent;

	block->version = MLX5_HV_VHCA_STATS_VERSION;
	block->rings   = mlx5e_get_netdev_max_channels(priv->netdev);
