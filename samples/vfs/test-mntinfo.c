...
#define FSINFO_OVERLARGE(X,Y)	 [FSINFO_ATTR_##X] = 0xc000
...
static void dump_attr_MOUNT_CHILD(union reply *r, int size)
{
	struct fsinfo_mount_child *f = &r->mount_child;

	printf("\t%8x %8x %8x\n", f->mnt_id, f->notify_counter, f->child_counter);
}
