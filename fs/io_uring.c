					  ...
					  bool needs_lock)
{
	...
	head = idr_find(&req->ctx->io_buffer_idr, gid);
	if (head) {
		if (!list_empty(&head->list)) {
			...
			kbuf = head;
			idr_remove(&req->ctx->io_buffer_idr, gid);
		}
	} else {
			       ...
			       int gid, unsigned nbufs)
{
	...
	i++;
	kfree(buf);
	idr_remove(&ctx->io_buffer_idr, gid);
...
static int io_remove_buffers(struct io_kiocb *req, bool force_nonblock)
{
	...
	ret = -ENOENT;
	head = idr_find(&ctx->io_buffer_idr, p->gid);
	if (head)
		ret = __io_remove_buffers(ctx, head, p->gid, p->nbufs);
...
static int io_provide_buffers(struct io_kiocb *req, bool force_nonblock)
{
	...
	list = head = idr_find(&ctx->io_buffer_idr, p->gid);
	...
	if (!list) {
		ret = idr_alloc(&ctx->io_buffer_idr, head, p->gid, p->gid + 1,
					GFP_KERNEL);
		if (ret < 0) {
			__io_remove_buffers(ctx, head, p->gid, -1U);
			goto out;
		}
	}
out:
