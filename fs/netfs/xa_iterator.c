...
{
	pgoff_t last = wback->last;
	int ret;

	_enter("%lx-%lx", wback->first, wback->last);
	ret = netfs_iterate_get_folios(wback->mapping, wback->first, wback->last,
				       netfs_lock_folios_iterator,
				       wback, may_wait);
	if (ret < 0) {
		netfs_see_writeback(wback, netfs_wback_trace_see_lock_conflict);
		goto failed;
	}

	if (wback->last < last) {
		kdebug("Some folios missing %lx < %lx", wback->last, last);
		netfs_see_writeback(wback, netfs_wback_trace_see_pages_missing);
		ret = -EIO;
		goto failed;
	}
					 ...
					 struct netfs_i_context *ctx)
{
	 ...
	 */
	if (within(&range, &wback->coverage))
		goto completely_inside;

	spin_lock(&ctx->lock);
	if (range.start < wback->coverage.start) {
		r = region = list_first_entry(&wback->regions,
	...
	if (range.end > wback->coverage.end) {
		r = region = list_last_entry(&wback->regions,
					     struct netfs_dirty_region, flush_link);
...
completely_inside:
	if (!folio_clear_dirty_for_io(folio))
		_debug("folio %lx is not dirty W=%x", folio_index(folio), wback->debug_id);
				     ...
				     pgoff_t first, pgoff_t last)
{
	...
	netfs_iterate_folios(wback->mapping, first, last,
			     netfs_mark_writeback_iterator, wback, ctx);
}
					...
					struct netfs_i_context *ctx)
{
	...
	if (!folio_test_writeback(folio)) {
		printk("\n");
		kdebug("WB NOT SET W=%x %lx", wback->debug_id, folio->index);
		return 0;
	}
...
void netfs_end_writeback(struct netfs_writeback *wback)
{
	...
	netfs_iterate_pinned_folios(wback->mapping, wback->first, wback->last,
				    netfs_end_writeback_iterator, wback, ctx);
}
