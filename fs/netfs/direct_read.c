...
ssize_t netfs_direct_read_iter(struct kiocb *iocb, struct iov_iter *iter)
{
	 ...
	 * iterator if we're going to access the pages from some other mm
	 * context.
	 */
	rreq->buffering = NETFS_DIRECT;
	rreq->direct_bv_mode = iov_iter_extract_mode(iter);
	if (rreq->direct_bv_mode)
	...
	case NETFS_DIRECT_BV:
		n = netfs_extract_user_iter(iter, rreq->len, &rreq->direct_iter);
		if (n < 0) {
			netfs_put_request(rreq, false, netfs_rreq_trace_put_discard);
			return n;
		}
		rreq->direct_bv = (struct bio_vec *)rreq->direct_iter.bvec;
