...
static int build_mount_kattr(unsigned int attr_set, unsigned int attr_clr,
			     ...
			     struct mount_kattr *kattr, unsigned int flags)
{
	...
	*kattr = (struct mount_kattr){
	...
	if (build_attr_flags(attr_set, &kattr->attr_set))
		return -EINVAL;

	if (build_attr_flags(attr_clr, &kattr->attr_clr))
		return -EINVAL;
	 ...
	 */
	if (!(attr_clr & MOUNT_ATTR__ATIME) && (attr_set & MOUNT_ATTR__ATIME))
		return -EINVAL;
	else if ((attr_clr & MOUNT_ATTR__ATIME) &&
		 ((attr_clr & MOUNT_ATTR__ATIME) != MOUNT_ATTR__ATIME))
		return -EINVAL;

	if (attr_clr & MOUNT_ATTR__ATIME) {
		/* Clear all previous time settings as they are mutually exclusive. */
		kattr->attr_clr |= MNT_RELATIME | MNT_NOATIME;
		switch (attr_set & MOUNT_ATTR__ATIME) {
		case MOUNT_ATTR_RELATIME:
...
SYSCALL_DEFINE6(mount_setattr, int, dfd, const char __user *, path,
		unsigned int, flags, unsigned int, attr_set,
		unsigned int, attr_clr, unsigned int, propagation)
{
	int err;
	struct path target;
	...
	if (attr_set == 0 && attr_clr == 0 && propagation == 0)
		return 0;

	err = build_mount_kattr(attr_set, attr_clr, propagation, &kattr, flags);
	if (err)
		return err;
