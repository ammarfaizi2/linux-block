 ...
 * known positive or ERR_PTR(). This is what most of the users want. Note that
 * pinned negative with unlocked parent _can_ become positive at any time, so
 * callers of lookup_one_unlocked() need to be very careful; pinned positives
 * have >d_inode stable, so this one avoids such problems.
 *
 * Note that this routine is purely a helper for filesystem usage and should
 * not be called by generic code.
 *
