...
static int cachefiles_cx_check_size(struct cachefiles_cache *cache)
{
	...
	if (ret < 0) {
		kerror("Unable to determine Backing FS dir FH size: error=%d",
		       ret);
		goto error_free;
	}
	...
	ret = exportfs_encode_fh(cache->cull_index->f_path.dentry, scratch,
				 &file_len, 1);
	if (ret < 0) {
		kerror("Unable to determine Backing FS file FH size: error=%d",
		       ret);
		goto error_free;
	}
...
static int cachefiles_cx_alloc_bitmap(struct cachefiles_cache *cache)
{
	...
	if (nslots != naslots) {
		kerror("Warning: index slots (%lu) does not match atime slots (%lu)\n",
		       nslots, naslots);
 ...
 */
static int cachefiles_cx_generate_actor(read_descriptor_t *desc,
					struct page *page,
					unsigned long offset,
					unsigned long size)
{
	struct cachefiles_cx_bitmap *last_bm = NULL;
	struct cachefiles_cache *cache = desc->arg.data;
	unsigned slot, nperpage = cache->cx_nperpage, index;
	u8 *kaddr, *p;
	struct cachefiles_cx_entry *ent;

	_enter(",{%lx},%lx,%lx", page->index, offset, size);

	if (offset != 0 || size != PAGE_SIZE || size > desc->count) {
		_leave(" = -EIO");
		return -EIO;
	}
...
int cachefiles_cx_generate_bitmap(struct cachefiles_cache *cache)
{
	...
	desc.count = i_size_read(cache->cull_index->f_path.dentry->d_inode);
	desc.arg.data = cache;
	pos = 0;
	_debug("begin scan");
	do_generic_file_read(cache->cull_index, &pos, &desc,
			     cachefiles_cx_generate_actor);
	_debug("end scan");
	ret = desc.error;
	if (ret < 0)
		goto error_free;
			 ...
			 struct dentry **_object)
{
	...
	if (IS_ERR(dentry)) {
		...
		kerror("Cannot decode cull slot %d FH: %d", slot, ret);
		return -EIO;
	}
