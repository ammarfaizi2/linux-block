...
lookup_error:
	ret = PTR_ERR(dentry);
	kerror("Lookup %s failed with error %d", filename, ret);
	mutex_unlock(&dir->d_inode->i_mutex);
	return ERR_PTR(ret);
				 ...
				 bool create)
{
	...
	if (!S_ISREG(dentry->d_inode->i_mode)) {
		kerror("%s is not a regular file", filename);
		ret = -EIO;
		goto check_error;
	}
	...
	_leave(" = [%lu]", file->f_dentry->d_inode->i_ino);
	return file;
...
create_error:
	dput(dentry);
	mutex_unlock(&dir->d_inode->i_mutex);
	kerror("mkdir %s failed with error %d", filename, ret);
	return ERR_PTR(ret);
