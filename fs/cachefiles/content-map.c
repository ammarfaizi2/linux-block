 ...
 */
u8 *cachefiles_new_content_map(struct cachefiles_object *object, size_t *_size)
{
	size_t size;
	u8 *map = NULL;
				 ...
				 unsigned int inval_counter)
{
	...
	if (unlikely(object->cookie->advice & FSCACHE_ADV_FALLBACK_IO))
		return;
...
int cachefiles_load_content_map(struct cachefiles_object *object)
{
	...
	/* Granulated object.  There's one bit per granule.  We size it in
	 * terms of 8-byte chunks, where a 64-bit span * 256KiB bytes granules
	 * covers 16MiB of file space.  At that, 512B will cover 1GiB.
	 */
	map = cachefiles_new_content_map(object, &size);
	if (!map)
		return -ENOMEM;

	cachefiles_begin_secure(cache, &saved_cred);
	got = vfs_getxattr(&init_user_ns, object->file->f_path.dentry,
...
void cachefiles_save_content_map(struct cachefiles_object *object)
{
	...
	ret = vfs_setxattr(&init_user_ns, object->file->f_path.dentry,
