...
static int cachefiles_daemon_add_cache(struct cachefiles_cache *cache)
{
	...
	if (IS_ERR(lockfile)) {
		...
		if (PTR_ERR(lockfile) == -ENOENT) {
			...
			kerror("Failed to check for lockfile.");
	...
	if (lockfile) {
		...
		if (IS_ERR(lockfile)) {
			kerror("Failed to create lockfile.");
			ret = PTR_ERR(lockfile);
			goto error_unsupported;
		}
	}
	...
	if (i_size_read(file_inode(cull_index)) & (PAGE_SIZE - 1)) {
		kerror("cull_index file not a multiple of PAGE_SIZE");
		cachefiles_mark_dirty(cache);
	}
	...
	if (i_size_read(file_inode(cull_atimes)) & (sizeof(__le32) - 1)) {
		kerror("cull_atimes file not a multiple of 4 (__le32)");
		cachefiles_mark_dirty(cache);
	}
...
void cachefiles_daemon_unbind(struct cachefiles_cache *cache)
{
	...
	if (cache->root && !test_bit(CACHEFILES_NEED_FSCK, &cache->flags)) {
		...
		if (IS_ERR(dentry)) {
			kerror("Error, could not lookup .lock file");
		} else if (dentry->d_inode) {
			_debug("Got handle to .lock file");
			ret = vfs_unlink(cache->root->d_inode, dentry);
			if (ret) kerror("Failed to delete .lock file.");
		} else {
