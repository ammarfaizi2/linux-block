...
bad_label_length:
	kerror("Cache cull index xattr length incorrect");
	ret = -EIO;
	goto error;

bad_label:
	xlabel[2] = 0;
	kerror("Cache cull index xattr specifies entry size of 0x%s not 0x%s",
	       xlabel, label);
	ret = -EIO;
	goto error;
}
...
int cachefiles_get_set_atime_base(struct cachefiles_cache *cache)
{
	...
	if (end - label != 16 || *end != '\0') {
		kerror("Failed to parse xattr on %*.*s [%lu]",
		       dentry->d_name.len, dentry->d_name.len,
		       dentry->d_name.name, dentry->d_inode->i_ino);
		ret = -EIO;
		goto error;
...
bad_label_length:
	kerror("Cache atime_base xattr length incorrect");
	ret = -EIO;
	goto error;
}
