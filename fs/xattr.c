...
generic_listxattr(struct dentry *dentry, char *buffer, size_t buffer_size)
{
	...
	if (!buffer) {
		for_each_xattr_handler(handlers, handler) {
			size += handler->xattr_list(dentry->d_inode, NULL, 0, NULL, 0,
					      handler->flags);
		}
	} else {
		char *buf = buffer;
		...
		for_each_xattr_handler(handlers, handler) {
			size = handler->xattr_list(dentry->d_inode, buf, buffer_size,
 ...
 */
int
generic_setxattr(struct inode *inode, const char *name, const void *value, size_t size, int flags)
{
	const struct xattr_handler *handler;
	...
	if (!handler)
		return -EOPNOTSUPP;
	return handler->xattr_set(inode, name, value, size, flags, handler->flags);
}
