...
void afs_server_timer(unsigned long data)
{
	struct afs_net *net = (struct afs_net *)data;

	if (!schedule_work(&net->server_reaper))
...
static void afs_set_server_timer(struct afs_net *net, time64_t delay)
{
	...
	if (net->live) {
		if (reduce_timer(&net->server_timer, jiffies + delay * HZ))
			afs_dec_servers_outstanding(net);
	} else {
...
void afs_reap_server(struct work_struct *work)
{
	...
	dec_after_work(&net->servers_outstanding);
}
