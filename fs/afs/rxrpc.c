...
int afs_open_socket(struct afs_net *net)
{
	...
	rxrpc_sock_set_kernel_socket(socket);
...
void afs_charge_preallocation(struct work_struct *work)
{
	...
	rxrpc_kernel_charge_accept(net->socket);
}
				    ...
				    unsigned long user_call_ID)
{
	...
	trace_afs_call(call->debug_id, afs_call_trace_discard,
		       refcount_read(&call->ref), o,
		       __builtin_return_address(0));
			    ...
			    unsigned long user_call_ID)
{
	...
	o = atomic_inc_return(&net->nr_outstanding_calls);
	trace_afs_call(call->debug_id, afs_call_trace_accept,
		       refcount_read(&call->ref), o,
		       __builtin_return_address(0));
