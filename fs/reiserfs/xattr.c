 ...
 */
ssize_t
reiserfs_getxattr(struct inode *inode, const char *name, void *buffer,
		  size_t size)
{
 ...
 */
int
reiserfs_setxattr(struct inode *inode, const char *name, const void *value,
			    ...
			    loff_t offset, u64 ino, unsigned int d_type)
{
	...
	if (name[0] != '.' ||
		...
		if (!handler)	/* Unsupported xattr name */
			return 0;
		if (b->buf) {
			size = handler->xattr_list(b->dentry->d_inode, b->buf + b->pos,
					 b->size, name, namelen,
					 handler->flags);
			if (size > b->size)
				return -ERANGE;
		} else {
			size = handler->xattr_list(b->dentry->d_inode, NULL, 0, name,
