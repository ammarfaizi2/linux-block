	...
	struct kiocb *iocb, struct iov_iter *from, bool direct)
{
	...
	if (user_backed_iter(from)) {
		/*
		 * Duplicate IOVEC/UBUF-type iterators as they contain
		 * references to the calling process's virtual memory layout
		 * which won't be available in an async worker thread.  This
		 * also takes a ref on every folio involved and attaches them
		 * to ctx->bv[].
		 */
		rc = netfs_extract_user_iter(from, iov_iter_count(from),
					     &ctx->iter, &ctx->bv_cleanup_mode);
		if (rc < 0) {
