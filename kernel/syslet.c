...
{
	memset(&tsk->acall, 0, sizeof(struct acall_submission));
...
int syslet_pre_indirect(void)
{
	...
	if (!syslet_frame_valid(&cur->acall.syslet.frame)) {
		ret = -EINVAL;
		goto out;
	}

	ring = (struct syslet_ring __user __force *)(unsigned long)
		cur->acall.syslet.completion_ring_ptr;
...
int syslet_post_indirect(int status)
{
	...
	struct task_struct *cur = current;
	struct syslet_args *args = &cur->acall.syslet;
...
void syslet_schedule(struct task_struct *cur)
{
	...
	if (child) {
		...
		set_user_frame(cur, &cur->acall.syslet.frame);
		cur->syslet_ready = 0;
