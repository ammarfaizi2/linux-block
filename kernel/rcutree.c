		...
		struct rcu_data __percpu *rda)
{
	...
	rsp->levelcnt[0] = num_rcu_lvl[0];
	rsp->levelcnt[1] = num_rcu_lvl[1];
	rsp->levelcnt[2] = num_rcu_lvl[2];
	rsp->levelcnt[3] = num_rcu_lvl[3];
	rsp->levelcnt[4] = num_rcu_lvl[4];
...
static void __init rcu_init_geometry(void)
{
	...
	rcu_capacity[2] = rcu_capacity[1] * CONFIG_RCU_FANOUT;
	rcu_capacity[3] = rcu_capacity[2] * CONFIG_RCU_FANOUT;
	rcu_capacity[4] = rcu_capacity[3] * CONFIG_RCU_FANOUT;
	...
	for (i = 1; i < MAX_RCU_LVLS + 1; i++)
		if (n <= rcu_capacity[i]) {
			for (j = 0; j <= i; j++)
				num_rcu_lvl[j] =
					DIV_ROUND_UP(n, rcu_capacity[i - j]);
			for (j = i + 1; j < MAX_RCU_LVLS + 1; j++)
				num_rcu_lvl[j] = 0;
			break;
		}
