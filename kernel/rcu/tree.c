 ...
 * @oldstate: Place to put state cookie
 *
 * Stores into @oldstate a value that will always be treated by functions
 ...
 *
 * @oldstate: value from get_state_synchronize_rcu_full() or start_poll_synchronize_rcu_full()
 *
 * If a full RCU grace period has elapsed since the earlier call from
 ...
 * counter wrap is harmless.  If the counter wraps, we have waited for more
 * than a billion grace periods (and way more on a 64-bit system!).  Those
 * needing to keep oldstate values for very long time periods (many hours
 * even on 32-bit systems) should check them occasionally and either refresh
 * them or set a flag indicating that the grace period has completed.
