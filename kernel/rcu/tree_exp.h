...
{
	struct rcu_node *rnp;
	bool serial = rcu_scheduler_active != RCU_SCHEDULER_RUNNING;
	...
	rcu_for_each_leaf_node(rsp, rnp) {
		...
		rnp->rew.rew_rsp = rsp;
		if (serial) {
			func(&rnp->rew.rew_work); /* No workqueues yet. */
			continue;
		}
		INIT_WORK(&rnp->rew.rew_work, sync_rcu_exp_select_node_cpus);
		queue_work_on(rnp->grplo, rcu_gp_workqueue, &rnp->rew.rew_work);
	}
