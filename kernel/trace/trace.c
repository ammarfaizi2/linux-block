...
static void *s_start(struct seq_file *m, loff_t *pos)
{
	...
	mutex_lock(&trace_types_lock);
	if (unlikely(old_tracer != tr->current_trace && tr->current_trace)) {
		old_tracer = tr->current_trace;
...
__tracing_open(struct inode *inode, struct file *file)
{
	...
	/* Currently only the top directory has a max_tr */
	if (tr->current_trace && tr->current_trace->print_max)
		iter->tr = &max_tr;
	else
		iter->tr = tr;
	iter->pos = -1;
	...
	if (ret < 0) {
		...
		if (cpu == RING_BUFFER_ALL_CPUS) {
			...
			for_each_tracing_cpu(i) {
				r = ring_buffer_resize(tr->buffer,
						per_cpu_ptr(tr->data, i)->entries,
						i);
				if (r < 0)
					break;
			}
...
create_trace_option_files(struct trace_array *tr);
...
static int tracing_set_tracer(const char *buf)
{
	...
	if (tr->current_trace && tr->current_trace->reset)
		tr->current_trace->reset(tr);
	if (tr->current_trace && tr->current_trace->use_max_tr) {
		/*
	...
	if (tr->current_trace->use_max_tr) {
		...
		for_each_tracing_cpu(cpu) {
			ret = ring_buffer_resize(max_tr.buffer,
						 per_cpu_ptr(global_trace.data, cpu)->entries,
						 cpu);
			if (ret < 0)
				goto out;
			per_cpu_ptr(max_tr.data, cpu)->entries =
				per_cpu_ptr(global_trace.data, cpu)->entries;
		}
	}
...
struct dentry *tracing_dentry_percpu(struct trace_array *tr, int cpu)
{
...
tracing_init_debugfs_percpu(struct trace_array *tr, long cpu)
{
	...
	trace_create_file("trace_pipe", 0444, d_cpu,
		  (void *)&per_cpu_ptr(data, cpu)->trace_cpu, &tracing_pipe_fops);

	/* per cpu trace */
	trace_create_file("trace", 0644, d_cpu,
		  (void *)&per_cpu_ptr(data, cpu)->trace_cpu, &tracing_fops);

	trace_create_file("trace_pipe_raw", 0444, d_cpu,
		  (void *)&per_cpu_ptr(data, cpu)->trace_cpu, &tracing_buffers_fops);

	trace_create_file("stats", 0444, d_cpu,
		  (void *)&per_cpu_ptr(data, cpu)->trace_cpu, &tracing_stats_fops);

	trace_create_file("buffer_size_kb", 0444, d_cpu,
		  (void *)&per_cpu_ptr(data, cpu)->trace_cpu, &tracing_entries_fops);
}
...
__init static int tracer_alloc_buffers(void)
{
	...
	if (!global_trace.data || !max_tr.data) {
		printk(KERN_ERR "tracer: failed to allocate ring buffer!\n");
		WARN_ON(1);
		goto out_free_cpumask;
	}
