...
static void clocksource_watchdog(struct timer_list *unused)
{
	...
	list_for_each_entry(cs, &watchdog_list, wd_list) {
		...
		if (wd_nsec < (wdi >> 2)) {
			pr_warn("timekeeping watchdog on CPU%d: Watchdog clocksource '%s' advanced %lld ns during %d-jiffy time interval, skipping watchdog check.\n", smp_processor_id(), watchdog->name, wd_nsec, WATCHDOG_INTERVAL);
			continue;
		}
		if (wd_nsec > (wdi << 2)) {
			pr_warn("timekeeping watchdog on CPU%d: Watchdog clocksource '%s' advanced %lld ns during %d-jiffy time interval, probable CPU overutilization, skipping watchdog check.\n", smp_processor_id(), watchdog->name, wd_nsec, WATCHDOG_INTERVAL);
			continue;
		}
