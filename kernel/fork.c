...
void __put_task_struct(struct task_struct *tsk)
{
	...
	WARN_ON(refcount_read(&per_task(tsk, usage)));
	WARN_ON(tsk == current);
...
#endif
	if (per_task(orig, cpus_ptr) == &per_task(orig, cpus_mask))
		per_task(tsk, cpus_ptr) = &per_task(tsk, cpus_mask);
...
static inline void rcu_copy_process(struct task_struct *p)
{
	...
	INIT_LIST_HEAD(&per_task(p, rcu_tasks_holdout_list));
...
#endif
	prev_cputime_init(&per_task(p, prev_cputime));
