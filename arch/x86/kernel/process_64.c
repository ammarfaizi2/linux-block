					     ...
					     enum which_selector which)
{
	if (likely(selector == 0)) {
		 ...
		 * X86_BUG_NULL_SEG CPUs.  If selector > 3, then it refers to
		 * a real segment, and saving the base isn't necessary.
		 */
		if (which == FS)
					    ...
					    enum which_selector which)
{
	if (likely(next_index <= 3)) {
		...
		/*
		 * The next task is using 32-bit TLS.  Loading the selector
		 * is sufficient.
		 */
		loadseg(which, next_index);
	}
