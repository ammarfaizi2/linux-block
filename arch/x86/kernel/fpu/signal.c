					    ...
					    struct _fpx_sw_bytes *fx_sw)
{
	...
	if (fx_sw->magic1 != FP_XSTATE_MAGIC1 ||
	    ...
	    fx_sw->xstate_size > task_thread(current).fpu->fpstate->user_size ||
	...
	if (likely(magic2 == FP_XSTATE_MAGIC2))
		return true;
setfx:
	trace_x86_fpu_xstate_check_failed(task_thread(current).fpu);
...
{
	if (use_fxsr()) {
		struct xregs_state *xsave = &task_thread(tsk).fpu->fpstate->regs.xsave;
		struct user_i387_ia32_struct env;
		struct _fpstate_32 __user *fp = buf;

		fpregs_lock();
		if (!test_thread_flag(TIF_NEED_FPU_LOAD))
			fxsave(&task_thread(tsk).fpu->fpstate->regs.fxsave);
		fpregs_unlock();
...
bool copy_fpstate_to_sigframe(void __user *buf, void __user *buf_fx, int size)
{
	...
	struct fpstate *fpstate = task_thread(tsk).fpu->fpstate;
...
{
	struct task_struct *tsk = current;
	struct fpu *fpu = task_thread(tsk).fpu;
	struct user_i387_ia32_struct env;
...
{
	unsigned long frame_size = xstate_sigframe_size(task_thread(current).fpu->fpstate);
