	...
	void *data)
{
	...
	case CU2_LWC2_OP:
		...
		if (insn.loongson3_lswc2_format.fr == 0) {	/* gslq */
			...
			set_fpr64(&task_thread(current).fpu.fpr[insn.loongson3_lswc2_format.rt], 0, value);
			set_fpr64(&task_thread(current).fpu.fpr[insn.loongson3_lswc2_format.rq], 0, value_next);
			compute_return_epc(regs);
			own_fpu(1);
		}
		return NOTIFY_STOP;	/* Don't call default notifier */
	...
	case CU2_SWC2_OP:
		...
		if (insn.loongson3_lswc2_format.fr == 0) {	/* gssq */
			...
			lose_fpu(1);
			value_next = get_fpr64(&task_thread(current).fpu.fpr[insn.loongson3_lswc2_format.rq], 0);

			StoreDW(addr + 8, value_next, res);
			if (res)
				goto fault;

			value = get_fpr64(&task_thread(current).fpu.fpr[insn.loongson3_lswc2_format.rt], 0);
	...
	case CU2_LDC2_OP:
		...
		case 0x6:
			...
			set_fpr64(&task_thread(current).fpu.fpr[insn.loongson3_lsdc2_format.rt], 0, value);
			compute_return_epc(regs);
			own_fpu(1);
	...
	case CU2_SDC2_OP:
		...
		case 0x6:
			...
			lose_fpu(1);
			value = get_fpr64(&task_thread(current).fpu.fpr[insn.loongson3_lsdc2_format.rt], 0);
