...
TRACE_EVENT(page_pool_flush,

	TP_PROTO(const struct page_pool *pool, int curr_nid),

	TP_ARGS(pool, curr_nid),

	TP_STRUCT__entry(
		__field(const struct page_pool *, pool)
		__field(int,			  pool_nid)
		__field(int,			  curr_nid)
	),

	TP_fast_assign(
		__entry->pool		= pool;
		__entry->pool_nid	= pool->p.nid;
		__entry->curr_nid	= curr_nid;
	),

	TP_printk("page_pool=%p pool_nid=%d curr_nid=%d",
		  __entry->pool, __entry->pool_nid, __entry->curr_nid)
);
