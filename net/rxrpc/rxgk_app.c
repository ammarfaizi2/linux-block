		       ...
		       u32 *_abort_code, const char **_eproto)
{
	...
	const struct rxgk_buffer *server_secret;
	...
	ret = rxgk_set_up_token_cipher(server_secret, &token_enc, enctype, &gk5e, GFP_NOFS);
	...
	*_eproto = tracepoint_string("rxgk_rsp_dec_tkt");
	ret = gk5e->scheme->decrypt_skb(NULL, gk5e, &token_enc, skb,
