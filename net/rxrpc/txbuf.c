...
struct rxrpc_txbuf *rxrpc_alloc_txbuf(struct rxrpc_call *call, gfp_t gfp)
{
	...
	if (txb) {
		...
		txb->wire.type		= RXRPC_PACKET_TYPE_DATA;
...
void rxrpc_get_txbuf(struct rxrpc_txbuf *txb, enum rxrpc_txbuf_trace what)
{
	...
	trace_rxrpc_txbuf(txb->call_debug_id, txb->seq, r + 1, what);
}
...
void rxrpc_see_txbuf(struct rxrpc_txbuf *txb, enum rxrpc_txbuf_trace what)
{
	...
	trace_rxrpc_txbuf(txb->call_debug_id, txb->seq, r, what);
}
...
void rxrpc_put_txbuf(struct rxrpc_txbuf *txb, enum rxrpc_txbuf_trace what)
{
	...
	if (txb) {
		...
		trace_rxrpc_txbuf(call_debug_id, seq, r - 1, what);
		if (dead)
