...
#define FIELD_COPY(dst, src, name)	((dst)->name = (src)->name)
	...
	/*
	 * if 'src' isn't holding a queue reference, get a live one here.
	 * that's safe from ->make_request_fn().
	 */
	if (!(src->rq_flags & RQF_HAS_Q_REF))
		blk_queue_enter_live(rq->q);
}
...
static bool blk_sq_sched_replenish(struct request_queue *q,
				   struct list_head *list)
{
#define REPLENISH_COUNT	8
	struct request *rq;
	int num_rq = 0;
...
static void blk_sq_sched_requeue(struct request_queue *q,
				 struct list_head *list)
{
	if (list_empty(list))
		return;
...
again:
	...
	if (!rq) {
		...
		if (!list_empty(&list) || blk_sq_sched_replenish(q, &list)) {
			sched_rq = list_first_entry(&list, struct request, queuelist);
			list_del_init(&sched_rq->queuelist);
		}
