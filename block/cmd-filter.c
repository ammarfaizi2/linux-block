...
int blk_cmd_verify(struct file *file, unsigned char *cmd)
{
	...
	if (test_bit(cmd[0], filter->write_ok) && (file->f_mode & FMODE_WRITE))
		return 0;

	return -EPERM;
}

EXPORT_SYMBOL(blk_cmd_verify);
...
static ssize_t rcf_writecmds_show(struct blk_scsi_cmd_filter *filter,char *page)
{
...
int blk_register_filter(struct gendisk *disk)
{
	...
	filter->kobj.parent = kobject_get(&disk->kobj);
	if (!filter->kobj.parent)
		return -EBUSY;

	snprintf(filter->kobj.name, KOBJ_NAME_LEN, "%s", "filter");
	filter->kobj.ktype = &rcf_ktype;

	ret = kobject_register(&filter->kobj);
	if (ret < 0)
		return ret;
...
void blk_unregister_filter(struct gendisk *disk)
{
	...
	kobject_unregister(&filter->kobj);
