...
static __wsum csum_and_copy_to_pipe_iter(const void *addr, size_t bytes,
					 struct iov_iter *i)
{
	struct pipe_inode_info *pipe = i->pipe;
	unsigned int p_mask = pipe->ring_size - 1;
					 ...
					 size_t maxsize, unsigned maxpages)
{
	...
	BUG_ON(k < 0); // if it had been empty, we'd get maxsize == 0

	if (unlikely(k))
		skip = 0;
	addr = (unsigned long)i->iov[k].iov_base + skip;
	len = i->iov[k].iov_len - skip;
	if (len > maxsize)
		len = maxsize;
				       ...
				       size_t maxsize, unsigned maxpages)
{
	...
	for (k = 0, skip = i->iov_offset; k < i->nr_segs; k++, skip = 0) {
		if (i->bvec[k].bv_len != skip)
			break;
	}
	BUG_ON(k == i->nr_segs); // if it had been empty, we'd get maxsize == 0

	len = i->bvec[k].bv_len - skip;
	if (len > maxsize)
		len = maxsize;
	skip += i->bvec[k].bv_offset;
	page = i->bvec[k].bv_page + skip / PAGE_SIZE;
	len += (*start = skip & (PAGE_SIZE - 1));
	if (len > maxpages * PAGE_SIZE)
		len = maxpages * PAGE_SIZE;
	*size = len;
	return page;
			     ...
			     struct iov_iter *i)
{
	...
	if (unlikely(iov_iter_is_pipe(i)))
		sum = csum_and_copy_to_pipe_iter(addr, bytes, i);
	...
	csstate->csum = csum_block_add(csstate->csum, sum, csstate->off);
	csstate->off += bytes;
	return bytes;
}
...
{
	size_t skip = i->iov_offset;
	const struct iovec *p, *end;
	int npages = 0;

	for (p = i->iov, end = p + i->nr_segs; p != end; p++) {
		unsigned long from = (unsigned long)p->iov_base;
		unsigned long to = from + p->iov_len;

		from += skip;
		skip = 0;
		if (unlikely(from == to))
			continue;
		npages += DIV_ROUND_UP(to, PAGE_SIZE) - from / PAGE_SIZE;
...
{
	size_t skip = i->iov_offset;
	const struct bio_vec *p, *end;
	int npages = 0;

	for (p = i->bvec, end = p + i->nr_segs; p != end; p++) {
		unsigned from = p->bv_offset;
		unsigned to = from + p->bv_len;
