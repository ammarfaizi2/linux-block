		      ...
		      const struct snd_pcm_hw_params *params)
{
	return find_format(&subs->fmt_list, params_format(params),
			   params_rate(params), params_channels(params), subs);
}
			     ...
			     struct snd_pcm_hw_params *hw_params)
{
	...
	ret = snd_usb_endpoint_configure(chip, subs->data_endpoint);
	if (ret < 0)
		goto unlock;
	if (ret > 0)
	...
	if (subs->sync_endpoint) {
		ret = snd_usb_endpoint_configure(chip, subs->sync_endpoint);
		if (ret < 0)
			goto unlock;
	}
