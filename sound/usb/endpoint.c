				       ...
				       struct snd_usb_endpoint *ep)
{
	...
	usb_audio_dbg(chip,
		      "Setting params for ep %x (type %s), rate=%d, format=%s, channels=%d, period_bytes=%d, periods=%d\n",
		      ep->ep_num, ep_type_name(ep->type), ep->cur_rate,
		      snd_pcm_format_name(ep->cur_format), ep->cur_channels,
