 ...
 *
 * TODO:
 *  - remove code duplication at various places.
...
void *q6asm_mmap_apr_reg(struct device *dev)
{
	if ((atomic_read(&this_mmap.ref_cnt) == 0) ||
	    (this_mmap.apr == NULL)) {
		this_mmap.apr = apr_register(dev, "ADSP", "ASM", \
...
pr_err("DEBUG::::::::::::::::::::; %s \n", __func__);
	...
	ac->apr = apr_register(dev, "ADSP", "ASM", \
	...
	ac->mmap_apr = q6asm_mmap_apr_reg(dev);
	if (ac->mmap_apr == NULL) {
		mutex_unlock(&session_lock);
		goto fail_mmap;
	}
...
int q6asm_audio_client_map_memory_regions(unsigned int dir, struct audio_client *ac,
					void *data, dma_addr_t phys, unsigned int bufsz, unsigned int bufcnt)
{
	int cnt = 0;
	int rc = 0;
	...
	pr_debug("DMA_DEBUG:%s: allocation sucess for %d bytes%p \n", __func__, bufsz*bufcnt, &buf[0].data);

	if (!buf[0].data) {
		pr_debug("%s:invalid vaddr, iomap failed\n", __func__);
		mutex_unlock(&ac->cmd_lock);
		goto fail;
	}
	buf[0].used = dir ^ 1;
	...
	return 0;
fail:
	q6asm_audio_client_unmap_memory_regions(dir, ac);
	return -EINVAL;
}
...
static int q6asm_memory_unmap_regions(struct audio_client *ac, int dir)
{
	...
	return q6asm_memory_unmap(ac, buf_add, dir);
}
...
static int q6asm_probe(struct platform_device *dev)
{
	int lcnt;
	pr_debug("%s:\n", __func__);
