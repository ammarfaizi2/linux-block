...
static unsigned long get_ctl_id_hash(const struct snd_ctl_elem_id *id)
{
	...
	h = (__force unsigned long)id->iface;
...
get_matched_num_entry(struct snd_card *card, unsigned int numid)
{
	return xa_load(&card->ctl_numids, numid);
}

static inline struct snd_kcontrol *
get_matched_ctl_entry(struct snd_card *card, const struct snd_ctl_elem_id *id)
{
	return xa_load(&card->ctl_hash, get_ctl_id_hash(id));
}
				...
				struct snd_kcontrol *kcontrol)
{
	...
	for (i = 0; i < kcontrol->count; i++) {
		...
		matched = get_matched_ctl_entry(card, &id);
		if (matched && (matched == kcontrol ||
				elem_id_matches(matched, &id)))
			xa_erase(&card->ctl_hash, get_ctl_id_hash(&id));
		id.index++;
		id.numid++;
	}
}
...
#endif /* CONFIG_SND_CTL_ELEM_HASH */
...
struct snd_kcontrol *snd_ctl_find_numid(struct snd_card *card, unsigned int numid)
{
	...
	if (IS_ENABLED(CONFIG_SND_CTL_ELEM_HASH))
		return get_matched_num_entry(card, numid);
				     ...
				     struct snd_ctl_elem_id *id)
{
	...
	kctl = get_matched_ctl_entry(card, id);
