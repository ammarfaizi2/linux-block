			       ...
			       struct evlist *evlist)
{
	...
	if (record__threads_enabled(rec)) {
		...
		if (ret)
			return ret;
		for (int i = 0; i < evlist->core.nr_mmaps; i++) {
			if (evlist->mmap)
...
{
	int tt, err, ret = 0, nr_threads = rec->nr_threads;
	struct record_thread *thread_data = rec->thread_data;
	sigset_t full, mask;
	pthread_t handle;
...
static int record__stop_threads(struct record *rec)
{
	...
	for (int t = 1; t < rec->nr_threads; t++)
		record__terminate_thread(&thread_data[t]);
...
{
	mask->bits = bitmap_zalloc(nr_bits);
	if (!mask->bits)
		return -ENOMEM;
...
{
	int ret, nr_cpus = perf_cpu_map__nr(cpus);
