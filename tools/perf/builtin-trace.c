...
struct trace {
	...
	char			*deleted_augmented_syscalls;
	struct {
...
static void trace__delete_augmented_syscalls(struct trace *trace)
{
	...
	trace->deleted_augmented_syscalls = strdup(bpf_object__name(trace->bpf_obj));
...
static int trace__run(struct trace *trace, int argc, const char **argv)
{
	...
	/*
	 * From now on we will not add any new event, so if no events are present
	 * we should bail out, not entering in an endless loop.
	 *
	 * Also if a BPF based augmented_syscalls was provided but deleted, because
	 * no syscall events was requested not --syscalls was asked for, warn the
	 * user about it.
	 */
	if (evlist->core.nr_entries == 0) {
		if (trace->deleted_augmented_syscalls) {
			fprintf(trace->output,
				"ERROR: Specify '--syscalls' or '-e list,of,syscall,names' to use with the provided\n\n"
				"  %s\n\n"
				"be it via -e or via the used .perfconfig 'trace.add_events'.\n",
				trace->deleted_augmented_syscalls);
		}
