...
{
	void *grnd_state;
	ssize_t(*vgetrandom)(void *state, void *buf, size_t len, unsigned long flags) = __vdsosym("LINUX_2.6", "__vdso_getrandom");
	void*(*vgetrandom_alloc)(size_t *num, size_t *size_per_each) = __vdsosym("LINUX_2.6", "__vdso_getrandom_alloc");
	unsigned int val;
	unsigned long times;
	...
	grnd_state = vgetrandom_alloc(&num, &size_per_each);
	if (!grnd_state)
	...
	for (times = 0; times < TRIALS; ++times) {
		ssize_t ret = vgetrandom(grnd_state, &val, sizeof(val), 0);
		if (ret != sizeof(val))
