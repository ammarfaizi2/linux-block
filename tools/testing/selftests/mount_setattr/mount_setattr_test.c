...
#ifndef MOUNT_PROPAGATION_SHARED
#define MOUNT_PROPAGATION_SHARED 4
#endif
...
{
	return syscall(__NR_mount_setattr, dfd, path, flags, attr_set, attr_clr, propagation);
}
...
{
	if (sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE,
			      MOUNT_ATTR_RDONLY | MOUNT_ATTR_NOSUID, 0,
			      MOUNT_PROPAGATION_SHARED))
...
{
	ASSERT_NE(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, (1U << 31), 0, 0), 0);

	ASSERT_NE(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, 0, (1U << 31), 0), 0);

	ASSERT_NE(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, 0, 0, (1U << 31)), 0);

	ASSERT_NE(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, (1U << 31),
				    (1U << 31), (1U << 31)), 0);
}

TEST_F(mount_setattr, basic)
{
	unsigned int attr_set = MOUNT_ATTR_RDONLY | MOUNT_ATTR_NOEXEC |
				MOUNT_ATTR_RELATIME,
		     attr_clr = MOUNT_ATTR__ATIME, propagation = 0;
	unsigned int old_flags = 0, new_flags = 0, expected_flags = 0;

	old_flags = read_mnt_flags("/mnt/A");
	ASSERT_GT(old_flags, 0);

	ASSERT_EQ(sys_mount_setattr(-1, "/mnt/A", 0, attr_set, attr_clr,
				    propagation), 0);
...
TEST_F(mount_setattr, basic_recursive)
{
	...
	ASSERT_EQ(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, attr_set,
				    attr_clr, propagation), 0);
	...
	propagation = MOUNT_PROPAGATION_SHARED;
	ASSERT_EQ(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, attr_set,
				    attr_clr, propagation), 0);
	...
	ASSERT_LT(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, attr_set,
				    attr_clr, propagation), 0);
...
{
	unsigned int attr_set = MOUNT_ATTR_RELATIME,
		     attr_clr = MOUNT_ATTR_RDONLY | MOUNT_ATTR_NOSUID |
	...
	ASSERT_EQ(sys_mount_setattr(-1, "/mnt/B", AT_RECURSIVE, attr_set,
				    attr_clr, propagation), 0);
...
{
	unsigned int attr_set = MOUNT_ATTR_NODIRATIME | MOUNT_ATTR_NOATIME,
		     attr_clr = 0, propagation = 0;
	unsigned int old_flags = 0, new_flags = 0, expected_flags = 0;

	ASSERT_NE(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, attr_set,
				    attr_clr, propagation), 0);
	...
	attr_set = MOUNT_ATTR_STRICTATIME | MOUNT_ATTR_NOATIME;
	ASSERT_NE(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, attr_set,
				    attr_clr, propagation), 0);
	...
	attr_set = 0;
	attr_clr = MOUNT_ATTR_STRICTATIME;
	ASSERT_NE(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, attr_set,
				    attr_clr, propagation), 0);

	attr_clr = MOUNT_ATTR_NOATIME;
	ASSERT_NE(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, attr_set,
				    attr_clr, propagation), 0);
	...
	ASSERT_EQ(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, 0, 0, 0), 0);
	...
	attr_set = 0;
	attr_clr = MOUNT_ATTR_NODIRATIME;
	propagation = 0;
	ASSERT_EQ(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, attr_set,
				    attr_clr, propagation), 0);
...
TEST_F(mount_setattr, wrong_user_namespace)
{
	...
	ret = sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, MOUNT_ATTR_RDONLY, 0, 0);
	ASSERT_LT(ret, 0);
...
TEST_F(mount_setattr, wrong_mount_namespace)
{
	...
	ret = sys_mount_setattr(fd, "", AT_EMPTY_PATH | AT_RECURSIVE,
				MOUNT_ATTR_RDONLY, 0, 0);
	ASSERT_LT(ret, 0);
