...
then
	...
	n_warn=`grep -v 'Warning: unable to open an initial console' | grep -v 'Warning: Failed to add ttynull console. No stdin, stdout, and stderr for the init process' $file | egrep -c 'WARNING:|Warn'`
	if test "$n_warn" -ne 0
	then
