...
do
	...
	runtest () {
		...
		then
			if ! grep -q '^Observation ' $dir/$1$2.out
			then
...
sort -k1n |
awk -v dq='"' -v ncpu=$LKMM_JOBS -v t=$T '
{
	print "if test -z " dq "$hwfnseg" dq " || scripts/simpletest.sh " $2
	print "then"
	print "\truntest " $2 " dq "$hwfnseg" dq " >> t "/" NR % ncpu ".sh";
	print "fi"
}
